  ...
  >
    <Form @submit="onSubmit" :validation-schema="schema" class="flex flex-column gap-3">
      ...
      <div class="flex justify-content-end gap-2 mt-3">
        ...
        />
        <AppButton type="submit" label="Salvar" />
      </div>
    </Form>
  </Dialog>
</template>
...
const schema = yup.object({
  name: yup.string().nullable().max(255, 'Nome deve ter no máximo 255 caracteres'),
  ...
  phoneNumber: yup
    ...
    .test('phone-digits', 'Telefone deve conter 10 ou 11 dígitos', (value) => {
      ...
      return phoneRegex.test(extractNumbers(value))
    }),
  birthDate: yup
    .date()
    .nullable()
...
async function onSubmit(values: IUpdateContact) {
  try {
    const payload: IUpdateContact = {
      ...values,
      phoneNumber: values.phoneNumber ? extractNumbers(values.phoneNumber) : undefined,
